...
//include_once('db_config.php');
  ...
  $db_sid = "(DESCRIPTION = (ADDRESS_LIST = (ADDRESS = (PROTOCOL = TCP)(HOST = THE_RAIN)(PORT = 1521)) ) (CONNECT_DATA = (SID = ali) ) )"; 
   ...
   if($con) 
	...
	$sql_update =
	"UPDATE Bed
				SET B_type = '$_POST[B_type]',
				W_id = '$_POST[W_id]'".
				"WHERE B_id ='$_REQUEST[B_id]'";

      $sql_select="SELECT B_id,B_type,W_id".
                  " from Bed".
                  " where B_id ='$_POST[B_id]' ";

      $sql_insert="insert into Bed(B_id,B_type,W_id)
                            values ('$_POST[B_id]','$_POST[B_type]','$_POST[W_id]')";
      $sql_Delete = "DELETE FROM Bed
					WHERE B_id = '$_POST[B_id]'";
		$Msg_Desc="";
      if ($_REQUEST[flag] == 1){
...
?>	
<div class="main_box">
		<div class="myDIV" align="center"><h1><font color=#5499C7> BED FORM </h1></div>
		<br>
						  <form  name="form1" method="POST" action="bed.php">
			<div class="subbox1"> 
		...
		<Table class ="table1" padding=10>
				  ...
				  <table align=center border="0" border-colour=black cellpadding="2" width="375"  >
					<tr>
						<td width="150" align="left">*Bed_ID</td>
						<td width="211"><input type="text" class="IN_box" name="B_id"  onfocus="" value=<?php echo $B_id?>></td>
					</tr>
					<tr>
						<td width="150" align="left">*B_Type</td>
						<td width="211">
						<input name="B_type" class="IN_box" value=<?php echo $B_type ?> ></td>
					</tr>			
					<tr>
